...
def stronglypos():
    ...
    if sptweets:
        ...
        df = pd.DataFrame(sptweets)
        df.to_csv('sptweets.csv', index=False, encoding='utf-8')
        f = open('sptweets.csv', 'r', encoding='UTF-8').read()
        f = f.lower()

        # wordcloud = WordCloud(max_font_size=50, max_words=100, background_color="white").generate(f)
        stopwords = set(STOPWORDS)
        stopwords.add("")
        stopwords.update(["positive", "rt", "strongly", "https", "negative", "neutral", "sentiment", "president", "co"])
        wordcloud = WordCloud(background_color="white", width=800, height=660, margin=2, stopwords=stopwords).generate(
            f)

        wordcloud.to_file("sptweets.jpg")
...
def pos():
    ...
    #         wr.writerow(ptweets)
            ...
            f = open('ptweets.csv', 'r', encoding='UTF-8').read()
            f = f.lower()
...
def stronglyneg():
    ...
    if sntweets:
        ...
        df = pd.DataFrame(sntweets)
        df.to_csv('sntweets.csv', index=False, encoding='utf-8')
        f = open('sntweets.csv', 'r',encoding='UTF-8').read()
        f = f.lower()
        stopwords = set(STOPWORDS)
        stopwords.add("")
        stopwords.update(["positive", "rt", "strongly", "https", "negative", "neutral", "sentiment", "president", "co"])
        ...
        wordcloud.to_file("sntweets.jpg")
...
def neu():
    ...
    # f = open('neutweets.csv', 'r',encoding='UTF-8').read()
    # f = f.lower()
    # stopwords = set(STOPWORDS)
    # stopwords.add("")
    # stopwords.update(["neutral", "rt", "strongly"])
    # wordcloud = WordCloud(background_color="white", width=800, height=660, margin=2, stopwords=stopwords).generate(f)
    # wordcloud.to_file("neutweets.jpg")
    if neutweets:
        # writing strongly negative tweets into a txt file for word cloud
        # with open('stronglyneg.txt', 'w') as f:
        #     for tweet in sntweets:
